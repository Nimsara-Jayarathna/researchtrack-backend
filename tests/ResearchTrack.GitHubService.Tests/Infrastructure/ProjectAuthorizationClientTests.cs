...
public sealed class ProjectAuthorizationClientTests
{
    ...
    [Fact]
    public async Task View_forwards_bearer_authentication_to_project_service()
    {
        ...
        await client.EnsureCanViewAsync(ProjectId, TestContext.Current.CancellationToken);
    ...
    [Fact]
    public async Task Manage_forwards_bearer_authentication_to_project_service()
    {
        var handler = new StubHandler(HttpStatusCode.OK);
        var accessor = new HttpContextAccessor { HttpContext = new DefaultHttpContext() };
        accessor.HttpContext.Request.Headers.Authorization = "Bearer access-token";
    ...
    [Fact]
    public async Task Inaccessible_project_is_reported_as_forbidden_for_view()
    {
        ...
        var exception = await Assert.ThrowsAsync<ApiException>(
            () => client.EnsureCanViewAsync(ProjectId, TestContext.Current.CancellationToken));

        Assert.Equal(StatusCodes.Status403Forbidden, exception.StatusCode);
        Assert.Contains("GitHub evidence", exception.Message);
    }
