...
public sealed class AuthUserDirectoryClient : IAuthUserDirectoryClient
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var envelope = await response.Content.ReadFromJsonAsync<ApiResponse<AuthDirectoryUser>>(
            cancellationToken: cancellationToken);

        if (!response.IsSuccessStatusCode || envelope?.Success != true || envelope.Data is null)
        {
            throw DependencyFailure("Unable to resolve the authenticated user from Auth Service.");
        }

        return envelope.Data;
    }

    public async Task<IReadOnlyList<AuthDirectoryUser>> ResolveStudentsAsync(
        IReadOnlyCollection<Guid> studentIds,
        CancellationToken cancellationToken)
    {
        using var request = new HttpRequestMessage(HttpMethod.Post, "api/v1/users/students/resolve")
        {
            Content = JsonContent.Create(new { studentIds })
        };
        ForwardAuthentication(request);

        using var response = await SendAsync(request, cancellationToken);
        var envelope = await response.Content.ReadFromJsonAsync<
            ApiResponse<IReadOnlyList<AuthDirectoryUser>>>(
                cancellationToken: cancellationToken);

        if (!response.IsSuccessStatusCode || envelope?.Success != true || envelope.Data is null)
        {
            throw DependencyFailure("Unable to validate selected students with Auth Service.");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw DependencyFailure("Auth Service is unavailable.", exception);
        }
        catch (TaskCanceledException exception) when (!cancellationToken.IsCancellationRequested)
        {
            throw DependencyFailure("Auth Service did not respond in time.", exception);
        }
    }
