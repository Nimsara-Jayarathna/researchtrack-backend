...
public sealed record JiraConnectionResponse(bool Connected, Guid ProjectId, string WorkspaceName, string? WorkspaceUrl, string JiraProjectId, string JiraProjectKey, string JiraProjectName, long? JiraBoardId, string? JiraBoardName, string? JiraBoardType, string SyncStatus, DateTimeOffset? LastSyncedAt, DateTimeOffset ConnectedAt, string WebhookStatus, DateTimeOffset? LastWebhookAt, DateTimeOffset? LastReconciledAt);

public sealed record JiraIssueResponse(string IssueKey,string Summary,string IssueType,bool IsSubtask,string Status,string? StatusCategory,string? Priority,string? AssigneeAccountId,string? AssigneeDisplayName,decimal? StoryPoints,string? ParentIssueKey,string? DescriptionJson,DateTimeOffset? DueDate,DateTimeOffset? CreatedAt,DateTimeOffset? UpdatedAt);
