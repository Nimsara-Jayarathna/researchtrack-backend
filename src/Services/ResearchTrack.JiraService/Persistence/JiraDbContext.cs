...
public sealed class JiraDbContext : DbContext
{
 ...
 public DbSet<JiraWebhookEvent> JiraWebhookEvents=>Set<JiraWebhookEvent>();
 ...
 protected override void OnModelCreating(ModelBuilder b)
 {
  ...
  b.Entity<JiraWebhookEvent>(e=>{e.ToTable("jira_webhook_events");e.HasKey(x=>x.Id);e.HasIndex(x=>x.DeliveryId).IsUnique();e.HasIndex(x=>new{x.Status,x.ReceivedAt});e.HasIndex(x=>x.ResearchProjectId);e.Property(x=>x.DeliveryId).HasMaxLength(255).IsRequired();e.Property(x=>x.CloudId).HasMaxLength(128).IsRequired();e.Property(x=>x.EventType).HasMaxLength(128).IsRequired();e.Property(x=>x.JiraIssueId).HasMaxLength(128);e.Property(x=>x.IssueKey).HasMaxLength(64);e.Property(x=>x.PayloadJson).HasColumnType("longtext");e.Property(x=>x.Status).HasMaxLength(32).IsRequired();e.Property(x=>x.LastError).HasColumnType("text");});
  b.Entity<JiraSyncJob>(e=>{e.ToTable("jira_sync_jobs");e.HasKey(x=>x.Id);e.HasIndex(x=>new{x.Status,x.AvailableAt});e.HasIndex(x=>new{x.ResearchProjectId,x.Status});e.Property(x=>x.Reason).HasMaxLength(64).IsRequired();e.Property(x=>x.Scope).HasMaxLength(32).IsRequired();e.Property(x=>x.EntityId).HasMaxLength(128);e.Property(x=>x.Status).HasMaxLength(32).IsRequired();e.Property(x=>x.LastError).HasColumnType("text");});
 }
}
