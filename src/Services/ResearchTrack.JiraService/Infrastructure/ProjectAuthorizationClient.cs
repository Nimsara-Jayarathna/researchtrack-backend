...
public sealed class ProjectAuthorizationClient : IProjectAuthorizationClient
{
    ...
    public async Task EnsureCanAccessAsync(Guid projectId,CancellationToken ct)
    {
        using var request=new HttpRequestMessage(HttpMethod.Get,$"api/v1/projects/{projectId}/authorization/access");ForwardAuthentication(request);
        HttpResponseMessage response;try{response=await _httpClient.SendAsync(request,ct);}catch(Exception ex) when(ex is HttpRequestException or TaskCanceledException){throw new ApiException(503,ErrorCodes.DependencyUnavailable,"Project Service is unavailable.",innerException:ex);}
        using(response){if(response.IsSuccessStatusCode)return;if(response.StatusCode==System.Net.HttpStatusCode.Unauthorized)throw new ApiException(401,ErrorCodes.Unauthorized,"Authentication is required.");if(response.StatusCode is System.Net.HttpStatusCode.Forbidden or System.Net.HttpStatusCode.NotFound)throw new ApiException(403,ErrorCodes.Forbidden,"You do not have access to this project.");throw new ApiException(503,ErrorCodes.DependencyUnavailable,"Unable to verify project authorization with Project Service.");}
    }
