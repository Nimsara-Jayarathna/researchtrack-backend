...
Service database files must provide `ConnectionStrings__DefaultConnection` with `Server=mysql`, `Port=3306`, and `SslMode=Disabled`; Docker DNS resolves `mysql` on the internal database network.
...
Database-service images contain an EF Core migration bundle at `/app/efbundle` and a `/app/migrate` wrapper. After MySQL is healthy, DB accounts are reconciled, and `/app/dbcheck` has proven service-credential connectivity, the deployment workflow runs `/app/migrate`; the wrapper passes `ConnectionStrings__DefaultConnection` to the EF bundle with `--connection` before reconciling the running application containers.
