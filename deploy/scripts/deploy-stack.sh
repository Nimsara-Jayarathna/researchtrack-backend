...
echo "[3/9] Start MySQL with current reconciliation script"
mysql_id="$("${compose[@]}" ps -a -q mysql)"
mysql_host_script_sha="$(sha256_file ./mysql/reconcile-databases.sh)"
mysql_force_recreate=false

if [[ -n "$mysql_id" ]]; then
  mysql_container_script_sha="$(container_file_sha256 "$mysql_id" /opt/researchtrack/reconcile-databases.sh || true)"
...
mysql_mounted_script_sha="$(container_file_sha256 "$mysql_id" /opt/researchtrack/reconcile-databases.sh || true)"
...
echo "[9/9] Reconcile application and observability containers"
...
if ((${#force_recreate_services[@]} > 0)); then
  "${compose[@]}" up -d --no-deps --force-recreate "${force_recreate_services[@]}"
fi
...
for service in "${app_services[@]}"; do
  container_id="$("${compose[@]}" ps -a -q "$service")"
  [[ -n "$container_id" ]] || { echo "Application container is missing after reconciliation: $service" >&2; exit 1; }
  running_image_id="$(docker inspect --format='{{.Image}}' "$container_id")"
  if [[ "$running_image_id" != "${app_target_image_ids[$service]}" ]]; then
...
"${compose[@]}" up -d --no-deps --force-recreate "${observability_services[@]}"

for service in "${observability_services[@]}"; do
  container_id="$("${compose[@]}" ps -a -q "$service")"
  [[ -n "$container_id" ]] || { echo "Observability container was not created: $service" >&2; exit 1; }
...
echo "Application images, environment changes, and observability configuration were reconciled successfully."
